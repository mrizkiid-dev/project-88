<template>
    ...
    <div class="px-2 flex flex-col gap-2 py-[100px]">
      <div v-if="isSearchLoading" class="flex justify-center">
          <Icon name="eos-icons:loading" size="25" />
      </div>
      <SearchSuggestion v-else-if="searchResult && searchResult.length > 0" v-for="card in searchResult" :key="card.id" :id="card.id" :title="card.title" :image-src="card.image" />
    </div>
    <Bottomnav />
</template>
